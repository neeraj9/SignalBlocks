...
TEST_CASE("Testing block average for scalar integer", "[average]") {
    ...
    std::shared_ptr<IPort<double> > block(new Average<double>("average-block-1"));
...
TEST_CASE("Testing math block average for vector integer", "[average]") {
    double input_value = M_PI;
    int len = 10;
    std::shared_ptr<IPort<double> > source(new ConstantSource<double, PORT_TYPE_VECTOR>("constant-1", input_value, len));
    std::shared_ptr<IPort<double> > block(new Average<double>("average-1"));
    std::shared_ptr<IPort<double> > sink(new ArchiverSink<double>("archive-1"));
    ArchiverSink<double>* archive = dynamic_cast<ArchiverSink<double>*>(sink.get());
...
TEST_CASE("Testing math block average for matrix integer", "[average]") {
    // no ctor args required
    double input_value = M_PI;
