...
#include "../blocks/math/ConstPow.h"
...
TEST_CASE("Testing block constant power for scalar integer", "[constant power]") {
    ...
    std::shared_ptr<IPort<double> > block(new ConstPow<double, 2>("constant-power-block-1"));
    ...
    double expected_result = input_value * input_value;
...
TEST_CASE("Testing math block constant power for vector integer", "[constant power]") {
    // no ctor args required
    double input_value = M_PI;
    int len = 10;
    std::shared_ptr<IPort<double> > source(new ConstantSource<double, PORT_TYPE_VECTOR>("constant-1", input_value, len));
    std::shared_ptr<IPort<double> > block(new ConstPow<double, 2>("constant-power-block"));
    std::shared_ptr<IPort<double> > sink(new ArchiverSink<double>("archive-1"));
    ArchiverSink<double>* archive = dynamic_cast<ArchiverSink<double>*>(sink.get());
...
TEST_CASE("Testing math block constant power for matrix integer", "[constant power]") {
    // no ctor args required
    double input_value = M_PI;
