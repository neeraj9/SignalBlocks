...
TEST_CASE("Testing block cos for scalar integer", "[cos]") {
    ...
    std::shared_ptr<IPort<double> > block(new Cos<double>("cos-block-1"));
    ...
    double expected_result = cos(input_value);
...
TEST_CASE("Testing math block cos for vector integer", "[cos]") {
    // no ctor args required
    double input_value = M_PI;
    int len = 10;
    std::shared_ptr<IPort<double> > source(new ConstantSource<double, PORT_TYPE_VECTOR>("constant-1", input_value, len));
    std::shared_ptr<IPort<double> > block(new Cos<double>("cos-1"));
    std::shared_ptr<IPort<double> > sink(new ArchiverSink<double>("archive-1"));
    ArchiverSink<double>* archive = dynamic_cast<ArchiverSink<double>*>(sink.get());
...
TEST_CASE("Testing math block cos for matrix integer", "[cos]") {
    // no ctor args required
    double input_value = M_PI;
