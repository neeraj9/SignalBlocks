...
#ifndef iblocks_math_NOperator_hh
#define iblocks_math_NOperator_hh
...
namespace iblocks
{
  ...
  protected:
    ...
    virtual void Compute(MultiPtr<T>& input,
                         int len,
                         MultiPtr<T>& output) const = 0;
    ...
    virtual void Compute(MultiPtr<T>& input1, int len1,
                         MultiPtr<T>& input2, int len2,
                         MultiPtr<T>& output) const = 0;
    ...
    //       it implies that its not present, so the algo needs to take care
    //       of that scenario as well by asumming all zeros.
    virtual void Compute(std::list<MultiPtr<T>*>& inputs,
                         std::list<int>& len,
                         MultiPtr<T>& output) const = 0;
#endif
