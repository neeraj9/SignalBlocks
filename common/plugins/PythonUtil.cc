...
PythonUtil::ExtractBasicType(PyObject* pObj)
{
  ...
  {
    return std::auto_ptr<PythonBaseResult>(0); // no result
  }
  // bool is a subclass of int so check it first.
  if (PyBool_Check(pObj))
  {
    long res = PyInt_AS_LONG(pObj);
    std::auto_ptr<PyBooleanResult> pres(new PyBooleanResult());
    if (res == 0)
    {
      pres->mValue = false;
    }
    ...
    return std::auto_ptr<PythonBaseResult>(pres.release());
  }
  else if (PyInt_Check(pObj))
  {
    ...
    std::auto_ptr<PyLongResult> pres(new PyLongResult());
    pres->mValue = res;
    return std::auto_ptr<PythonBaseResult>(pres.release());
  }
  else if (PyLong_Check(pObj))
  {
    ...
    if (PyErr_Occurred())
    {
      ...
      std::auto_ptr<PyLongLongResult> pres(new PyLongLongResult());
      pres->mValue = res;
      return std::auto_ptr<PythonBaseResult>(pres.release());
    }
    // else
  ...
  else if (PyFloat_Check(pObj))
  {
    ...
    std::auto_ptr<PyDoubleResult> pres(new PyDoubleResult());
    pres->mValue = res;
    return std::auto_ptr<PythonBaseResult>(pres.release());
  }
  else if (PyComplex_Check(pObj))
  {
    ...
    std::auto_ptr<PyComplexResult> pres(new PyComplexResult());
  ...
  else if (PyString_Check(pObj))
  {
    ...
    // buffer of the pObj
    std::auto_ptr<PyStringResult> pres(new PyStringResult());
...
PythonUtil::ExtractSequenceType(PyObject* pObj)
{
  ...
  if (PyTuple_Check(pObj))
  {
    ...
    std::auto_ptr<PyTupleResult> pres(new PyTupleResult());
    for (int i = 0; i < len; ++i)
    {
      ...
      std::auto_ptr<PythonBaseResult> new_element(ExtractBasicType(pobj));
  ...
  else if (PyList_Check(pObj))
  {
    ...
    std::auto_ptr<PyListResult> pres(new PyListResult());
    for (int i = 0; i < len; ++i)
    {
  ...
  else if (PyDict_Check(pObj))
  {
    ...
    PyObject* value;
    Py_ssize_t pos = 0;
    std::auto_ptr<PyDictResult> pres(new PyDictResult());
    while (PyDict_Next(pObj, &pos, &key, &value))
    {
      if (key !=0 && value != 0)
      {
        std::auto_ptr<PythonBaseResult> pkey(ExtractBasicType(key));
        std::auto_ptr<PythonBaseResult> pvalue(ExtractBasicType(value));
  ...
  return std::auto_ptr<PythonBaseResult>(0);
}
